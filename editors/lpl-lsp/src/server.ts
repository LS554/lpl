...
function parseDocumentSymbols(text: string): SymbolInfo[] {
...
function getIncludedHeaders(text: string): Set<string> {
    ...
    while ((m = re1.exec(text)) !== null) {
        ...
        let h = m[1];
        if (h.endsWith('.lph')) h = h.slice(0, -4);
        headers.add(h);
    }
...
function resolveClass(name: string): [string, ClassDoc] | undefined {
    if (STDLIB[name]) return [name, STDLIB[name]];
    if (name === 'List') return ['List<T>', STDLIB['List<T>']];
    if (name === 'Map') return ['Map<K,V>', STDLIB['Map<K,V>']];
    return undefined;
}

function formatMethodDoc(className: string, methodName: string, doc: MethodDoc): string {
    let md = '```lpl\n' + `${className}.${methodName}: ${doc.signature}` + '\n```\n\n';
    md += doc.description + '\n';
    if (doc.params && doc.params.length > 0) {
        md += '\n**Parameters:**\n';
        for (const p of doc.params) md += `- \`${p}\`\n`;
    }
    if (doc.returns) md += `\n**Returns:** \`${doc.returns}\`\n`;
    return md;
}

function formatClassDoc(className: string, cls: ClassDoc): string {
    let md = '```lpl\nclass ' + className + '\n```\n\n';
    md += cls.description + '\n';
    md += `\n*Header:* \`<${cls.header}>\`\n`;
...
// ---------------------------------------------------------------------------
// Diagnostics via compiler --check
// ---------------------------------------------------------------------------
...
function runValidation(doc: TextDocument): void {
    ...
    execFile(compilerPath, [tmpFile, '--check'], { timeout: 10000 }, (error, stdout, _stderr) => {
        const diagnostics: Diagnostic[] = [];

        try {
            const diags: CompilerDiag[] = JSON.parse(stdout || '[]');
...
connection.onCompletion((params: CompletionParams): CompletionItem[] => {
        ...
        if (resolved && headers.has(resolved[1].header)) {
            ...
            for (const [name, mdoc] of Object.entries(cls.methods)) {
                items.push({
                    ...
                    documentation: { kind: MarkupKind.Markdown, value: formatMethodDoc(displayName, name, mdoc) },
                });
            }
        }
        for (const sym of symbols) {
            if (sym.className === className && (sym.kind === 'method' || sym.kind === 'constructor')) {
                items.push({ label: sym.name, kind: CompletionItemKind.Method, detail: sym.signature || sym.name });
            }
            if (sym.className === className && sym.kind === 'field') {
                items.push({ label: sym.name, kind: CompletionItemKind.Field, detail: `${sym.type} ${sym.name}` });
            }
        }
    ...
    if (lineText.match(/include\s+<\s*$/)) {
        for (const h of ['console', 'string', 'math', 'files', 'system', 'collections', 'sync']) {
            items.push({ label: h, kind: CompletionItemKind.File, detail: 'LPL standard library header' });
        }
        return items;
    }
    ...
    for (const kw of ALL_KEYWORDS) {
        const item: CompletionItem = { label: kw, kind: CompletionItemKind.Keyword };
        if (KEYWORD_DOCS[kw]) {
            item.documentation = { kind: MarkupKind.Markdown, value: KEYWORD_DOCS[kw] };
        }
        items.push(item);
    }
    ...
    for (const [name, cls] of Object.entries(STDLIB)) {
        ...
        items.push({
            ...
            documentation: { kind: MarkupKind.Markdown, value: formatClassDoc(name, cls) },
        });
    }

    for (const sym of symbols) {
        if (sym.kind === 'function') {
            items.push({ label: sym.name, kind: CompletionItemKind.Function, detail: sym.signature });
        } else if (sym.kind === 'class') {
            items.push({ label: sym.name, kind: CompletionItemKind.Class, detail: `class ${sym.name}` });
        } else if (sym.kind === 'variable') {
            items.push({ label: sym.name, kind: CompletionItemKind.Variable, detail: `${sym.type} ${sym.name}` });
        }
    }
...
connection.onHover((params: TextDocumentPositionParams): Hover | null => {
    ...
    if (classMatch) {
        ...
        if (resolved && headers.has(resolved[1].header)) {
            ...
            if (method) {
                return { contents: { kind: MarkupKind.Markdown, value: formatMethodDoc(resolved[0], word, method) } };
            }
        }
        const userMethod = symbols.find(s => s.className === classMatch[1] && s.name === word && (s.kind === 'method' || s.kind === 'constructor'));
        if (userMethod) return { contents: { kind: MarkupKind.Markdown, value: formatSymbolHover(userMethod) } };
        const userField = symbols.find(s => s.className === classMatch[1] && s.name === word && s.kind === 'field');
        if (userField) return { contents: { kind: MarkupKind.Markdown, value: formatSymbolHover(userField) } };
    }
    ...
    if (instanceMatch) {
        ...
        if (varSym) {
            ...
            if (resolved && headers.has(resolved[1].header)) {
                const method = resolved[1].methods[word];
                if (method) return { contents: { kind: MarkupKind.Markdown, value: formatMethodDoc(resolved[0], word, method) } };
            }
        }
    }
    ...
    if (resolved && headers.has(resolved[1].header)) {
        return { contents: { kind: MarkupKind.Markdown, value: formatClassDoc(resolved[0], resolved[1]) } };
    }

    // User-defined symbol
    const userSym = symbols.find(s => s.name === word);
    if (userSym) {
        return { contents: { kind: MarkupKind.Markdown, value: formatSymbolHover(userSym) } };
    }
...
connection.onSignatureHelp((params: TextDocumentPositionParams): SignatureHelp | null => {
    ...
    const sig: SignatureInformation = {
        ...
        parameters: (method.params || []).map(p => {
            const parts = p.split(' — ');
            return { label: parts[0], documentation: parts[1] || '' } as ParameterInformation;
        }),
    };
